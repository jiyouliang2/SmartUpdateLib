...
#include "com_itheima_updatelib_PatchUtil.h"
...
JNIEXPORT jint JNICALL Java_com_itheima_updatelib_PatchUtil_patch(
		JNIEnv *env, jstring old, jstring news, jstring patch) {
	...
	__android_log_print(ANDROID_LOG_INFO, "ApkPatchLibrary", "old = %s ",
			ch[1]);
	__android_log_print(ANDROID_LOG_INFO, "ApkPatchLibrary", "news = %s ",
			ch[2]);
	__android_log_print(ANDROID_LOG_INFO, "ApkPatchLibrary", "patch = %s ",
			ch[3]);

	int ret = applypatch(4, ch);

	__android_log_print(ANDROID_LOG_INFO, "ApkPatchLibrary",
			"applypatch result = %d ", ret);
